...
public class ClubService {
    ...
    public List<Player> getInterestedPlayersForClub(Club club) {
        ...
        return playerRepository.findPlayersByClubsToJoinContaining(club.getId());
    }
    ...
    public Transfer createTransfer(Long clubId, Long playerId) {
        ...
        try {
            ...
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Failed to save transfer");
        }
    ...
    @Transactional
    public void approveTransferByCoach(Long transferId) {
        Transfer transfer = transferRepository.findById(transferId).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Transfer not found"));
        transfer.setTransferStatus(TransferStatus.COACH_APPROVED);
    }

    @Transactional
    public void deleteTransferByCoach(Long transferId) {
        try {
